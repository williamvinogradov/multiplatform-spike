import {ItemVM} from '../types';
import {FAKE_ITEM_LABEL, VISIBLE_PAGE_COUNT} from './consts';


const getFakePageItem = (value: number, fakeTemplate: unknown): ItemVM => {
  return {
...
const getPageItemsVmShort = (selectedPage: number, pageCount: number, template: unknown): ItemVM[] => {
  const result: ItemVM[] = [];
