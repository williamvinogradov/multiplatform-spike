...
describe('Core: Component', () => {
  ...
  describe('Initialization', () => {
    it('creates state', () => {
      const expectedState = { model: {}, dictionary: {} };

      createCoreComponent(expectedState, {}, {});

      expect(createStateMock).toHaveBeenCalledTimes(1);
      expect(createStateMock).toHaveBeenCalledWith(expectedState);
    });

    it('creates reducer', () => {
      const expectedActionHandlers = {};
      const secondReducerFuncMock = jest.fn();
      createReducerMock.mockReturnValue(secondReducerFuncMock);

      createCoreComponent(stateValueMock, {}, expectedActionHandlers);

      expect(createReducerMock).toHaveBeenCalledTimes(1);
      expect(secondReducerFuncMock).toHaveBeenCalledTimes(1);
      expect(secondReducerFuncMock).toHaveBeenCalledWith(expectedActionHandlers);
    });

    it('creates validation pipe', () => {
      const expectedValidation = [jest.fn()];

      createCoreComponent(stateValueMock, {}, {}, expectedValidation);

      expect(pipeMock).toHaveBeenCalledTimes(1);
      expect(pipeMock).toHaveBeenCalledWith(...expectedValidation);
    });
  });

  describe('component.commitUpdates', () => {
    it('calls state\'s base method', () => {
      const [rootCore] = createCoreComponent(stateValueMock, {}, {});
      rootCore.commitUpdates();
  ...
  describe('component.dispatch', () => {
    it('calls reducer', () => {
      ...
      const [, containerCore] = createCoreComponent(
        stateValueMock,
        {},
        { [stateModelProp]: jest.fn() },
      );
      containerCore.dispatch(stateModelProp, expectedValue);
    ...
    it('calls validator', () => {
      ...
      containerCore.dispatch(stateModelProp, {});
  ...
  describe('component.addUpdate', () => {
    it('calls state\'s base method', () => {
      const expectedUpdate = {};
      const [rootCore] = createCoreComponent(stateValueMock, {}, {});
      rootCore.addUpdate(expectedUpdate);
  ...
  describe('component.rollbackUpdates', () => {
    it('calls state\'s base method', () => {
      const [rootCore] = createCoreComponent(stateValueMock, {}, {});
      rootCore.rollbackUpdates();
