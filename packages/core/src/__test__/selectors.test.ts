import { createCacheSelector } from '../selectors';
...
describe('Core: Selectors', () => {
  describe('createCacheSelector', () => {
    it('passes arguments to memoize', () => {
      const func = jest.fn();
      const comparer = jest.fn();

      createCacheSelector(jest.fn(), func, comparer);
    ...
    it('returns memoized', () => {
      ...
      const selector = createCacheSelector(jest.fn(), jest.fn(), jest.fn());
    ...
    it('passes params to memoized', () => {
      ...
      const selector = createCacheSelector(getParams, jest.fn(), jest.fn());
      selector({});
